...
from akana.tokens import FS


class AkTable(QFrame):
    """Bordered wrap + QTableWidget styled as .ak-table.

    Expands to the full content column width so Stretch columns are usable
    (not squeezed to sizeHint).
    """
    ...
    def set_columns(self, columns: list[str]) -> None:
        ...
        header.setFont(font)
        # Stretch every column across the full wrap width
    ...
    def sizeHint(self) -> QSize:  # noqa: N802
        # Prefer a readable width when unconstrained; still expands with parent
        sh = super().sizeHint()
        return QSize(max(sh.width(), 480), sh.height())
